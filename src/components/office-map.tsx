...
export function OfficeMap({ data, activeRoom, onRoom, onMove, onMember, onJoin, onProfile, onStatus, onSettings, onHand, onReaction, reaction, call, onOpenCall }: Props) {
  ...
  return <section ref={container} className={`office-card ${fullscreen ? "office-expanded" : ""}`} aria-label="Mapa do escritório virtual">
    ...
    <div className="office-tabs" aria-label="Ambientes do escritório"><button className={activeRoom === "all" ? "active" : ""} onClick={() => onRoom("all")}><Grid2X2 size={14}/>Visão geral</button>{data.rooms.filter(r => ["recepcao", "coworking", "lounge"].includes(r.id)).map(r => <button key={r.id} className={activeRoom === r.id ? "active" : ""} onClick={() => onRoom(r.id)}><RoomIcon kind={r.kind} size={14}/>{r.id === "lounge" ? "Lounge" : r.name}</button>)}<span className="office-tabs-spacer"/><span className="floor-caption">ANDAR 01</span></div>
