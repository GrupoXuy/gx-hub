...
import { Building2, Video, Users, CalendarDays, ChevronDown, ChevronRight, ChevronsUpDown, ArrowUpRight, UserPlus, Search, Bell, CircleHelp, Menu, X, Clock3, UserRoundPen, Plus, Smartphone, Layers3, Sprout, Globe2, Wifi, WifiOff, Settings2, Sparkles, Check, Info, PhoneOff, Maximize2, Headphones, ArrowRight, Command } from "lucide-react";
...
type Dialog = { type: "profile" | "invite" | "schedule" | "settings" | "help" | "search" } | { type: "member"; member: Member } | { type: "meeting"; meeting: Meeting } | { type: "join"; room: Room } | { type: "company"; name?: string } | null;
...
export default function WorkspaceApp() {
  const { data, setData, connected, error, refresh, updateMe } = useWorkspace();
  ...
  const [clock, setClock] = useState(""); const [reaction, setReaction] = useState(""); const [invited, setInvited] = useState(false);
  ...
  const inviteChecked = useRef(false); const inviteTarget = useRef<string | null>(null); const lastMessage = useRef<string | null>(null);
  ...
  useEffect(() => {
    if (data.me.id === "local" || inviteChecked.current) return;
    inviteChecked.current = true;
    const params = new URLSearchParams(window.location.search); const token = params.get("invite"); if (!token) return;
    void api(`/api/invites?token=${encodeURIComponent(token)}`).then(() => { inviteTarget.current = params.get("room"); setInvited(true); setDialog({ type: "profile" }); }).catch(err => notify(err instanceof Error ? err.message : "Convite inválido."));
  }, [data.me.id, notify]);
  useEffect(() => {
  ...
  const saveProfile = async (value: Partial<Member>) => { await updateMe(value); setDialog(null); notify(invited ? "Bem-vindo ao Grupo X! Seu lugar na equipe está pronto." : "Seu avatar foi atualizado. Sua essência, em cada conexão."); if (invited) { setInvited(false); const url = new URL(window.location.href); url.searchParams.delete("invite"); url.searchParams.delete("room"); window.history.replaceState({}, "", url); const room = data.rooms.find(r => r.id === inviteTarget.current); if (room) setDialog({ type: "join", room }); } };
  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${sidebarOpen ? "sidebar-open" : ""}`}><button className="brand" onClick={() => navigate("office")} aria-label="GX Hub — início"><BrandMark size={43}/><span><strong>GX <b>HUB</b></strong><small>VIRTUAL WORKSPACE</small></span></button>
      ...
      <nav className="main-nav" aria-label="Navegação principal"><span className="nav-group-label">SEU WORKSPACE</span>{NAV.map(item => <button key={item.id} className={`nav-item ${view === item.id ? "active" : ""}`} onClick={() => navigate(item.id)}><item.icon size={18} strokeWidth={1.65}/><span>{item.label}</span>{item.id === "team" ? <span className="nav-counter">{data.members.length}</span> : view === item.id ? <span className="nav-active-dot"/> : null}</button>)}</nav>
      <div className="ecosystem-nav"><span className="nav-group-label">NOSSO ECOSSISTEMA<span className="tiny-x">✧</span></span>{[{ name: "Senna Cell X", icon: Smartphone, className: "cell" }, { name: "Grupo Reis X", icon: Layers3, className: "reis" }, { name: "Primeiro Passo X", icon: Sprout, className: "passo" }].map(company => <button className="ecosystem-nav-item" key={company.name} onClick={() => setDialog({ type: "company", name: company.name })}><span className={`company-nav-icon ${company.className}`}><company.icon size={14} strokeWidth={1.5}/></span><span>{company.name}</span><ArrowUpRight size={12}/></button>)}<button className="ecosystem-explore" onClick={() => setDialog({ type: "company" })}>Explorar ecossistema<ArrowRight size={12}/></button></div>
      <div className="sidebar-bottom"><div className="sidebar-manifesto"><span className="manifesto-symbol">✳</span><p>Mais que um escritório.<br/><strong>Um ponto de conexão.</strong></p><span className="manifesto-background-x">X</span></div><button className="connection-state" onClick={() => setDialog({ type: "help" })}>{connected ? <Wifi size={13}/> : <WifiOff size={13}/>}<span>{connected ? "Conexão estável" : "Conectando ao escritório"}</span><span className={`small-status-dot ${connected ? "available" : "away"}`}/></button><button className="sidebar-invite" onClick={() => setDialog({ type: "invite" })}><UserPlus size={16}/>Convidar pessoas<Plus size={13}/></button><button className="sidebar-user" onClick={() => setDialog({ type: "profile" })}><Avatar member={data.me} size={35} status/><span><strong>{data.me.name}</strong><small>{data.me.role}</small></span><Settings2 size={15}/></button></div>
    </aside>
    <div className="workspace-main"><header className="topbar"><div className="topbar-left"><IconButton label="Abrir menu" className="mobile-menu-button" onClick={() => setSidebarOpen(true)}><Menu size={21}/></IconButton><span className="breadcrumb-workspace">Workspace</span><ChevronRight size={13} className="breadcrumb-chevron"/><span className="breadcrumb-current">{NAV.find(n => n.id === view)?.label}</span></div><div className="topbar-right"><button className="topbar-search" aria-label="Buscar no workspace" onClick={() => setDialog({ type: "search" })}><Search size={16}/><span>Buscar</span><kbd>⌘ K</kbd></button><span className="header-divider"/><span className="topbar-clock"><Clock3 size={14}/><strong>{clock || "--:--"}</strong><span>BRT</span></span><IconButton label="Como funciona o GX Hub" onClick={() => setDialog({ type: "help" })}><CircleHelp size={18}/></IconButton><button className={`icon-button notification-button ${notificationsOpen ? "selected" : ""}`} aria-label="Notificações" aria-expanded={notificationsOpen} onClick={() => setNotificationsOpen(!notificationsOpen)}><Bell size={18}/>{!notificationsRead && <span className="notification-dot"/>}</button><span className="header-divider"/><button className="header-group-brand" onClick={() => setDialog({ type: "company", name: "Grupo X" })}>GRUPO <span>X</span></button></div>
    ...
    {dialog?.type === "profile" && <ProfileDialog me={data.me} invited={invited} onSave={saveProfile} onClose={close}/>}
    ...
    {dialog?.type === "meeting" && <MeetingDialog meeting={dialog.meeting} data={data} onJoin={openJoin} onCancelled={() => { close(); void refresh(); notify("Reunião cancelada. O horário está disponível novamente."); }} onClose={close}/>}
    {dialog?.type === "member" && <MemberDialog member={data.members.find(m => m.id === dialog.member.id) || dialog.member} rooms={data.rooms} own={dialog.member.id === data.me.id} onProfile={() => setDialog({ type: "profile" })} onVisit={visit} onJoin={openJoin} onClose={close}/>}
