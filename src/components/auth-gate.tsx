...
export function AuthGate({ inviteToken, onLoginEmail, onRegister }: Props) {
  ...
  const submitRegister = async (event: FormEvent) => {
    event.preventDefault(); setBusy(true); setError("");
    try { await onRegister({ name: name.trim(), role: role.trim(), company, color, gender, inviteToken: invite.trim(), email: email.trim(), password }); } catch (err) { setError(err instanceof Error ? err.message : "Não foi possível concluir o cadastro."); } finally { setBusy(false); }
  };
