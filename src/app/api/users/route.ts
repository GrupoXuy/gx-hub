...
export async function POST(request: Request) {
  try {
    ...
    const canAccessGroupSystem = body.canAccessGroupSystem === true;
    ...
    const [member] = await db.insert(users).values({
      ...
      roomId: "recepcao", status: "available", x: 61, y: 73,
       isDemo: false, isAdmin: body.isAdmin === true, canAccessGroupSystem, accessToken: randomToken(), lastSeen: new Date(0),
    }).returning();
...
export async function PATCH(request: Request) {
  try {
    ...
    if (typeof body.canAccessGroupSystem === "boolean") {
      patch.canAccessGroupSystem = body.canAccessGroupSystem;
    }
