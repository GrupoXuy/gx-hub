...
import { Building2, Video, Users, CalendarDays, ChevronRight, ChevronsUpDown, ArrowUpRight, UserPlus, Search, Bell, CircleHelp, Menu, X, Clock3, UserRoundPen, Plus, Smartphone, Layers3, Sprout, Wifi, WifiOff, Settings2, Sparkles, Check, Info, PhoneOff, Maximize2, Headphones, ArrowRight, ShieldCheck, LogOut } from "lucide-react";
...
type Dialog = { type: "profile" | "invite" | "schedule" | "settings" | "help" | "search" | "users" } | { type: "member"; member: Member } | { type: "meeting"; meeting: Meeting } | { type: "join"; room: Room } | { type: "company"; name?: string } | null;
...
export default function WorkspaceApp() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content"><div className="page-heading"><div><p className="eyebrow"><span />{heading.eyebrow}</p><h1>{heading.title}</h1><p className="page-description">{heading.description}</p></div><div className="page-heading-actions">{view === "office" ? <button className="button button-secondary avatar-edit-button" onClick={() => setDialog({ type: "profile" })}><UserRoundPen size={16} />Personalizar avatar</button> : view === "team" ? (data.me.isAdmin ? <button className="button button-primary" onClick={() => setDialog({ type: "users" })}><ShieldCheck size={16} />Gerenciar usuários</button> : <button className="button button-primary" onClick={() => setDialog({ type: "invite" })}><UserPlus size={16} />Convidar pessoas</button>) : <button className="button button-primary" onClick={() => setDialog({ type: "schedule" })}><Plus size={17} />Agendar reunião</button>}</div></div>
